// Database layer with native PostgreSQL support and local fallback store
import { Pool } from 'pg';
import fs from 'fs';
...
if (connectionString) {
  try {
    ...
    console.warn('[DB] Could not initialize PostgreSQL Pool, falling back to local file storage:', err.message);
  }
}
  ...
  try {
    ...
    try {
      ...
      if (adminCheck.rows.length === 0) {
        // Default password hash for 'ThienAn@CIO2026!'
        await client.query(`
...
export async function insertMatchLog({ userName, userEmail, companyName, jobTitle, jdFilename, jdText, matchScore, aiResult, ipAddress }) {
  if (usePostgres && pool) {
    const res = await pool.query(
      ...
      [userName, userEmail, companyName, jobTitle, jdFilename, jdText, matchScore, JSON.stringify(aiResult), ipAddress]
    );
    return res.rows[0];
  } else {
...
export async function getMatchLogs(limit = 100) {
  if (usePostgres && pool) {
    const res = await pool.query('SELECT * FROM jd_match_logs ORDER BY created_at DESC LIMIT $1', [limit]);
    return res.rows;
  } else {
    const store = readLocalStore();
...
export async function getMatchLogById(id) {
  if (usePostgres && pool) {
    const res = await pool.query('SELECT * FROM jd_match_logs WHERE id = $1', [id]);
...
export async function getAppointments(limit = 100) {
  if (usePostgres && pool) {
    const res = await pool.query('SELECT * FROM appointments ORDER BY created_at DESC LIMIT $1', [limit]);
    return res.rows;
  } else {
    const store = readLocalStore();
...
export async function updateAppointmentStatus(id, status) {
  if (usePostgres && pool) {
    const res = await pool.query('UPDATE appointments SET status = $1 WHERE id = $2 RETURNING *', [status, id]);
    return res.rows[0];
  } else {
    const store = readLocalStore();
...
export async function getResumeRequests(limit = 100) {
  if (usePostgres && pool) {
    const res = await pool.query('SELECT * FROM resume_requests ORDER BY created_at DESC LIMIT $1', [limit]);
    return res.rows;
  } else {
    const store = readLocalStore();
...
export async function getSystemSettings() {
  if (usePostgres && pool) {
    const res = await pool.query('SELECT value FROM system_settings WHERE key = $1', ['app_config']);
...
export async function updateAdminPassword(email, newHashedPassword) {
  if (usePostgres && pool) {
    const res = await pool.query('UPDATE admin_users SET password_hash = $1 WHERE email = $2 RETURNING id, email, name', [newHashedPassword, email]);
    return res.rows[0];
  } else {
    const store = readLocalStore();
